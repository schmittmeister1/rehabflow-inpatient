    ...
    <div className="app-layout">
      ...
      {showNewAdmission && <NewAdmissionModal onClose={()=>setShowNewAdmission(false)} onSave={(form)=>{
        ...
        var newPatient = {
          ...
          ggSelfCare: { A: { admission: 3, goal: 5, discharge: null }, B: { admission: 3, goal: 5, discharge: null }, C: { admission: 3, goal: 5, discharge: null }, D: { admission: 3, goal: 5, discharge: null }, E: { admission: 2, goal: 4, discharge: null }, F: { admission: 3, goal: 5, discharge: null }, G: { admission: 2, goal: 4, discharge: null }, H: { admission: 2, goal: 4, discharge: null }, I: { admission: 3, goal: 5, discharge: null } },
...
function Schedule({ patients, setSelectedPatient, setCurrentPage, setNavigationSource, customAppointments, setCustomAppointments }) {
  ...
  const to12h = function(t) { var p = t.split(':'); var h = parseInt(p[0]); var ap = h >= 12 ? 'PM' : 'AM'; h = h % 12 || 12; return h + ':' + p[1] + ' ' + ap; };

  const scheduleForDate = useMemo(() => {
    if (isToday) {
      var todayAppts = [...SCHEDULE_DATA];
      if (customAppointments && customAppointments.length > 0) {
        ...
        customAppointments.forEach(ca => {
          if (ca.date === dateKey) {
            ...
            if (pt) {
              for (var oi = todayAppts.length - 1; oi >= 0; oi--) { if (todayAppts[oi].time === to12h(ca.time) && todayAppts[oi].therapist === ca.therapist && !todayAppts[oi].patientId) todayAppts.splice(oi, 1); }
              todayAppts.push({time: to12h(ca.time), therapist: ca.therapist, patientId: ca.patientId, patient: pt.lastName + ', ' + pt.firstName, type: ca.type, status: 'Scheduled'});
            }
          }
        });
      }
    ...
    if (customAppointments && customAppointments.length > 0) {
      ...
      customAppointments.forEach(ca => {
        if (ca.date === dateKey) {
          const pt = patients.find(p => p.id == ca.patientId);
          if (pt) {
              for (var oi = generated.length - 1; oi >= 0; oi--) { if (generated[oi].time === to12h(ca.time) && generated[oi].therapist === ca.therapist && !generated[oi].patientId) generated.splice(oi, 1); }
              generated.push({time: to12h(ca.time), therapist: ca.therapist, patientId: ca.patientId, patient: pt.lastName + ', ' + pt.firstName, type: ca.type, status: 'Scheduled'});
            }
        }
      });
    }
...
function WeeklyScheduleView({ patients, selectedDate, setSelectedPatient, setCurrentPage, setNavigationSource, customAppointments }) {
  ...
  var getApptsForDay = function(dayDate) {
    ...
    var generated = [];
    var activePatients = patients.filter(function(p) { return p.status !== 'Discharged'; });
    if (isToday && typeof SCHEDULE_DATA !== 'undefined') {
      SCHEDULE_DATA.forEach(function(a) { generated.push(a); });
    } else {
      var dayOffset = Math.floor((dayDate - new Date()) / 86400000);
      timeSlots.forEach(function(slot, idx) {
        var seed1 = ((idx + 1) * 7 + dayOffset * 13 + 1) % 0x7FFFFFFF;
        var seed2 = ((idx + 1) * 11 + dayOffset * 17 + 2) % 0x7FFFFFFF;
        if (seed1 % 100 < 65 && activePatients.length > 0) {
          var pIdx = seed1 % activePatients.length;
          var p = activePatients[pIdx];
          var type = p.careStage === 'Admission (Day 1)' ? 'Initial Eval' : 'Treatment';
          generated.push({time: slot.label, therapist: 'PT', patientId: p.id, patient: p, type: type, status: 'Scheduled'});
        }
        if (seed2 % 100 < 45 && activePatients.length > 0) {
          var pIdx2 = seed2 % activePatients.length;
          generated.push({time: slot.label, therapist: 'PTA', patientId: activePatients[pIdx2].id, patient: activePatients[pIdx2], type: 'Treatment', status: 'Scheduled'});
        }
      });
    }
    if (customAppointments) {
